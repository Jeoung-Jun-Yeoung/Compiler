...
A_lD *searchIdentifierAtCurrentLevel(char *,A_ID *);
...
A_TYPE *getTypeOfSstructOrEnumRefIdentifier(T_KIND,char *,ID_KIND);
...
void setDefaultSpeciﬁer(A_SPECIFIER *p) {
	A_TYPE *t
	if (p->type==NIL)
		p->type=int_type;
	if (p->stor==S_NULL)
...
A ID *setDeclaratorListSpecifier(A_ID *id, A_SPECIFIER *p) {
	A_ID *a;
	setDefaultSpecifier(p);
	a=id;
	while (a) {
...
A_TYPE *setTypeNameSpecifier(A_TYPE *t, A_SPECIFIER *p) {
	...
	if (p->stor)
		syntax_error(20);
	setDefaultSpecifier(p);
	t=setTypeElementType(t,ip->type);
	return(t);
}
...
BOOLEAN isNotSameFormaIParameters(A_ID *a, A_id *b){
	if (a==NIL)// no parameters in Prototype
		return(FALSE);
	while(a) {
		if (b==NILL isNotSameType(a->type,b->type))
			return (TRUE);
		a=a—>link;
		b=b—>link; }
...
BOOLEAN isNotSameType(A_TYPE *t1, A_TYPE xt2) {
	if (isPointerOrArrayType(t1)||isPointerOrArrayType(t2))
		return(isNotSameType(tl->element_type,t2->element_type));
	else 
		return (t1!=t2);
}
...
void initialize() {
	...
	int_type=setTypeAndKindOfDeclarator(
		makeType(T_ENUM),ID_TYPE,makeldentifier("int"));
	float_type=setTypeAndKindOfDeclarator(
		makeType(T_ENUM),ID_TYPE,makeldentifier("float"));
	char_type= setTypeAndKindOfDeclarator(
		makeType(T_ENUM),ID_TYPE,makeldentifier("char"));
	void_type=setTypeAndKindOfDeclarator(
		makeType(T_VOID),ID_TYPE,makeldentifier("void"));
	...
	setDeclaratorTypeAndKind(
		...
		setTypeField(
			setTypeElementType(makeType(T_FUNC),stnng_type),
...
void syntax_error(int i,char *s) {
	...
	switch (i){
		...
		case 26: printf("illegal or conflicting retern type in function %s",s); break;
		case 31: printf("undefine type for identifier %s",s); break;
